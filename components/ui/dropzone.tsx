    ...
    return (
      <div className={cn('flex flex-row items-center gap-x-2 justify-center', className)}>
        ...
        <p className="text-primary text-sm">
          {t('successfullyUploaded', { count: files.length })}
        </p>
      </div>
    )
  }
          ...
          >
            ...
            <div className="shrink grow flex flex-col items-start truncate">
              ...
              {file.errors.length > 0 ? (
                ...
                <p className="text-xs text-destructive">{t('failedToUpload', { error: fileError.message })}</p>
              ) : isSuccessfullyUploaded ? (
                <p className="text-xs text-primary">{t('successfullyUploaded', { count: 1 })}</p>
              ) : (
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn('flex flex-col items-center gap-y-2', className)}>
      ...
      <p className="text-sm">
        {t('uploadPrompt', { maxFiles: maxFiles || 0 })}
      </p>
      <div className="flex flex-col items-center gap-y-1">
        <p className="text-xs text-muted-foreground">
          ...
          >
            {t('selectFiles', { maxFiles: maxFiles || 0 })}
          </a>{' '}
          {t('toUpload')}
        </p>
        {maxFileSize !== Number.POSITIVE_INFINITY && (
          <p className="text-xs text-muted-foreground">
            {t('maximumFileSize', { size: formatBytes(maxFileSize, 2) })}
          </p>
        )}
      </div>
    </div>
